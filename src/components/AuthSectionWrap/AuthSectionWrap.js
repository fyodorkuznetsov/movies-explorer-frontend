...
function AuthSectionWrap(props) {
  const buttonClass = `auth-section__btn ${!props.valid ? 'auth-section__button_state_disabled' : ''}`;
  ...
  return (
    <main className="auth-section">
      ...
      <form className="auth-section__form" action="#" encType="multipart/form-data" method="POST" noValidate onSubmit={props.onSubmit}>
        ...
        <div className="auth-section__form-footer">
          <button type="submit" className={`auth-section__button ${buttonClass}`} disabled={!props.valid}>{props.buttonText}</button>
